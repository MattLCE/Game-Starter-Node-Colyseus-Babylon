...
export const PlayerInput = defineComponent({ left: Types.ui8, right: Types.ui8, forward: Types.ui8, backward: Types.ui8 });
...
export class MyRoom extends Room<MyRoomState> {
    ...
    onJoin(client: Client, _options: any) {
        try {
            ...
            Position.x[eid] = Math.random() * 10 - 5; Position.y[eid] = 1.0; Position.z[eid] = Math.random() * 10 - 5;
            ...
            console.log(`[MyRoom] Created ECS entity ${eid} and associated Rapier body object for client ${client.sessionId}`);
    ...
    update(deltaTime: number) {
        if (!this.state || !this.ecsWorld || !this.rapierWorld || !this.playerQuery) { return; }
        ...
        } catch (e) { console.error("[MyRoom Update] Error during input processing:", e); return; }

        // --- Step Physics Simulation ---
        try { this.rapierWorld.step(); }
        catch (e) { console.error("[MyRoom Update] Error during Rapier step:", e); return; }
                ...
                if (hasComponent(this.ecsWorld, Position, eid)) {
                 ...
                 if (clientId) {
                    ...
                    if (playerState && hasComponent(this.ecsWorld, Position, eid)) {
                         const posX = Position.x[eid]; const posY = Position.y[eid]; const posZ = Position.z[eid];
                         if (posX !== undefined && posY !== undefined && posZ !== undefined) {
                             playerState.x = posX; playerState.y = posY; playerState.z = posZ;
                         }
                     }
                 }
            }
        } catch (e) { console.error("[MyRoom Update] Error during state sync:", e); return; }
        ...
        // --- Handle Player State Additions/Removals (Colyseus State side) ---
         ...
         } catch (e) { console.error("[MyRoom Update] Error during player join sync:", e); return; }
         ...
         } catch (e) { console.error("[MyRoom Update] Error during player leave sync:", e); return; }
    }
     ...
     onLeave(client: Client, _consented: boolean) {
        ...
        if (eid !== undefined) {
            ...
            console.warn(`[MyRoom] Client ${client.sessionId} left, but no matching entity found in clientEntityMap.`);
            return;
        }
        ...
        try {
            ...
            if (rigidBody) {
                 console.log(`[MyRoom] Found Rapier body object via eid ${eid}. Removing...`);
                 ...
                 for (const collider of collidersToRemove) { this.rapierWorld.removeCollider(collider, false); }

                 this.rapierWorld.removeRigidBody(rigidBody);
                 console.log(`[MyRoom] Successfully removed Rapier body object for eid ${eid}.`);
                 ...
                 // Keep this warning
                 console.warn(`[MyRoom] Could not find Rapier body in eidToRapierBodyMap for eid ${eid}. Cannot remove physics body.`);
            }

            // --- Remove ECS Components ---
             console.log(`[MyRoom] Marking ECS components for removal for eid ${eid}.`);
             ...
             console.error(`!!! ERROR during onLeave processing for eid: ${eid} !!!`, leaveError);
        }
    }
    ...
    private findClientIdByEid(eid: number): string | undefined {
        for (const [clientId, entityId] of this.clientEntityMap.entries()) {
            if (entityId === eid) { return clientId; }
        }
        return undefined;
    }
